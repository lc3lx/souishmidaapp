...
class SMMController {
  ...
  static async syncProviderServices(provider) {
    try {
      ...
      provider.supportedServices = services.map(service => {
        // ابحث عن الخدمة القديمة لنفس serviceId
        const oldService = Array.isArray(provider.supportedServices)
          ? provider.supportedServices.find(s => s.serviceId === service.service.toString())
          : null;
        return {
          ...
          isActive: true,
          adminProfit: oldService && typeof oldService.adminProfit === 'number' ? oldService.adminProfit : 0
        };
      });
  ...
  // Update adminProfit for a specific service in a provider
  static async updateServiceAdminProfit(req, res) {
    try {
      const { providerId, serviceId, adminProfit } = req.body;
      if (!providerId || !serviceId || typeof adminProfit !== 'number') {
        return res.status(400).json({
          success: false,
          message: 'providerId, serviceId, و adminProfit مطلوبة.'
        });
      }
      const provider = await SMMProvider.findById(providerId);
      if (!provider) {
        return res.status(404).json({
          success: false,
          message: 'مقدم الخدمة غير موجود.'
      ...
      const service = provider.supportedServices.find(s => s.serviceId === serviceId);
      if (!service) {
        return res.status(404).json({
          success: false,
          message: 'الخدمة غير موجودة ضمن مزود الخدمة.'
        });
      }
      service.adminProfit = adminProfit;
      await provider.save();
      res.json({
        success: true,
        message: 'تم تحديث هامش الربح بنجاح.',
        data: { service }
      });
    } catch (error) {
      console.error('Update adminProfit error:', error);
      res.status(500).json({
        success: false,
        message: 'خطأ في تحديث هامش الربح.'
      });
    }
  }
}
